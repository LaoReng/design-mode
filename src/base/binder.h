...
public:
    ...
    CBinder(_OBJ *sub, _FUNC func)
        : m_func(FuncType<_FUNC>::MakeFunc(func)), m_pObj(sub), m_pFunc(*(void **)&func)
    {
    }
    template <typename _FUNC>
    CBinder(_FUNC func)
        : m_func(FuncType<_FUNC>::MakeFunc(func)), m_pFunc(*(void **)&func)
    {
    }
    // INFO: 之前不能写成构造函数的原因是，我们需要模板类传回来所封装函数的参数列表，而我们现在还不需要这个可变参数所以不用在函数列表里面写但是需要在模板上写，这就造成了必须通过其他函数的形式把函数的模板类型显示的给出来
    template <typename _OBJ, typename _FUNC>
